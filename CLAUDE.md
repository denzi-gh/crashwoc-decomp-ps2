...
6. **Verification** - `verify_promoted.py` re-derives every `matching` claim (full registry extent, size = gap to next function) and `link_image.py` links the full image and compares the retail SHA. CI does not gate on progress regressions and never writes back to the repo; the per-run report artifact is the only progress output (accumulate it externally, e.g. decomp.dev, for a trend).
...
CI (docs/pipeline.md, "CI" section): `validate.yml` (structure, no game files, every push/PR) and `matching.yml` (full byte gates, runs inside a private GHCR image with game files baked in, dtk-template style).

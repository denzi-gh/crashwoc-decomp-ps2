...
them. Note that the decomp.dev treemap only renders units with `total_code > 0`,
so the data units are correctly counted in the report and category metrics but do
not appear as clickable tiles — expected, not a bug.

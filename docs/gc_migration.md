...
| `game/jeep`    | 7         | 7       | 5     | 2       | 0        | JeepMatrix/WheelPoint helpers + FireBoss-objective getters + Draw*LevelExtra/Temp + LimitSpeedZbyXZ. FireBoss.HitPoints@0x408 (extern struct). BlendNUVECs near-match (jal delay-slot fill: int daddu vs float mov.s). DrawWesternArenaLevelExtra inlines an EnemyJeep-struct loop (deferred); big Jeep/FireBoss/Trail state fns pending |
